...
Every record also carries a **`screen`** field: the live client stamps each signal with the screen
the page belongs to (the Markdown file's basename, exposed as `window.__islesScreen`). A consumer can
use it to scope a record to a specific screen — rejecting, say, a click from a stale tab that lands
after a newer screen was pushed. You don't set `screen` yourself; the client adds it.

Consumers tail `<dir>/state/events` — e.g. a host can block on a `proceed` record for a given `screen`,
or watch for its own pack-defined types.
